 ...
 */
function generate_html_data( $path ) {

	if ( ! is_html_file_exists( $path ) ) {
		return split_html( '<html><head></head><body><!-- データを取得できません --></body></html>' );
	}

	return split_html( get_html_data( $path ) ) ?? split_html( '<html><head></head><body><!-- データが取得できません --></body></html>' );
}
 ...
 */
function get_html_data( $path ) {
	$html_contents = file_get_contents( $path );
	if ( $html_contents ) {
		return convert_relative_paths_to_absolute( $html_contents, get_contents_url( $path ) );
	}
	return '<html><head></head><body><!-- データが変換できませんでした --></body></html>';
}
 ...
 */
function get_contents_url( $path ) {
